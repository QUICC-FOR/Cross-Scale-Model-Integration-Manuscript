...
\item Current interest in modeling potential changes to species ranges have resulted in a great diversity of approaches to species distribution modeling.
...
\item Our framework uses a metamodel, estimated in a hierarchical Bayesian framework, that is constrained by both data directly applied to the metamodel and by the results of one or more sub-models.
...
\item We illustrate our approach using two examples, and demonstrate that the framework can substantially reduce uncertainty when projecting beyond the range of the original data.
\item We conclude by discussing the application of our method and its accessibility to conservation biologists and land managers.
...
Finally, we provide a more formal mathematical presentation in Box 1.
...
We apply our framework here to such heterogeneous sources of information to reduce the bias in parameter estimation and more adequately represent uncertainty (see Appendix 1 in Supporting Information for procedural details and scripts for executing the model).
...
Here we describe briefly the dataset, methods, and the results of the analysis (see Appendix 2 for data and scripts to reproduce the analysis).
...
These variables were selected from the initial set of 6 variables from \citet{Morin2009}; see Appendix 2 for details on model construction and selection.
...
Because the metamodel is expressed as a series of conditional probabilities (see Box 1), this information can be included simply provided the probability of the metamodel, conditional on information from the sub-models, can be expressed mathematically.
...
All data, as well as all code required to repeat the analyses, have been uploaded as online supporting information.
...
\begin{figure}[h!]
	...
	\caption{Results of model integration for sugar maple.
