...
\dg{I think we should drop her, now that she is not in science anymore}
...
Current interests in forecasting potential changes to species ranges have resulted in a great diversity of approaches to species distribution modeling.
...
We provide a more formal mathematical presentation in the Appendix S1, along with complete code and data for both examples (Appendix S2).
...
\dg{I dropped the 'bayesian' from this sentence because I was looking at the posterior in the following equation and only found the likelihood. It comes in the following equation.} 
...
\dg{you specify the algorithm of the sampler below, but not here}
...
\dg{There is something wrong with the following equation, or I missed the reasonning somewhere in the last year. In any case, I tried to understand the logic from the text and haven't succeed at it. Either it needs to be fixed, or it needs to be better explained. If I don't get it, my prior tells me nobody will. This equation is central to the paper now that the maths have been put in the appendix. I was expecting to see the likelihood of X_M given the parameters theta_M.  The prior of theta_M being the posterior of the model theta_M applied to the data psi_S. The actual likelihood gives the probability of observing the prediction of the sub-model given the parameters of the sub-model. There is not integration on the conditions, it only results from the product of the probabilities.}
...
\dg{This is a vague statement. Could you give just an extra sentence specifying how you did it, and then pointing to the Appendix?}
...
\dg{might worth increasing font size on the figures. And make sure it is the same for all figures.}
...
The challenge is three fold: (i) which spatial and temporal scales (i.e. which processes), are to be considered, (ii) how to build a scale the sub-models to be consistent with the metamodels, and (iii) how error and uncertainty propagate from the sub-models to the metamodel. 
...
\dg{I also think that there is the challenge inherent to complex systems that is overlooked to a certain extent here. Small scale processes might generate organization at higher scales. Model integration might be limited by this problem in some cases, while in others it might solve it by putting bounds on the higher-order organization. Think about the evaluation of model parameters with observations at the scale of the operating process. There are some parameter values, within the posterior distribution, that could result in non-desirable properties at the larger scale. Model integration could buffer this propagation.}
