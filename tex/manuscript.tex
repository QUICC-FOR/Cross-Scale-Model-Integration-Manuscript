...
\begin{titlingpage}
	...
	\textbf{Main text word count: } 4848 %WORD_COUNT %Target 5000
...
We illustrate our approach using theoretical and empirical examples, and demonstrate that the framework better characterizes uncertainty and can substantially reduce uncertainty when projecting beyond the range of the original data.
...
However, the link between different sub-models is based on assumptions about the scaling of ecological processes that are poorly known and difficult to test \citep{Gallien2010}, and uncertainties are approximated and their attribution to different sources is difficult to distinguish.
...
Contrary to hybrid methods, the aim is not to link different sub-models into a single model, but to condition the predictions of a metamodel at the target scale (e.g., an entire species' range) with information from independent sub-models at a variety of spatial scales, allowing for more flexibility regarding the type of information included. 
...
We provide a more formal mathematical presentation (Appendix S1) along with complete code and data for both examples (Appendix S2).
...
We desire to mo0del the species' distribution as a function of temperature, \(T_M\), and precipitation, \(P_M\).
...
The parameters for the model were estimated using a Metropolis-Hastings algorithm.
...
However, the possibility that diverse approaches will provide diverging answers as a result of different assumptions and methodologies can create confusion and mistrust towards models, and some managers may be discouraged from incorporating model results in their management plans. 
...
In many cases, both the data and theory needed to apply our approach already exist, and all that is needed is the development of sub-models and the links between the sub-models and a metamodel.
...
\ib{you say in the intro that the bayesian framework allows a easy and simple scaling-up, but the scale-up is still a problem. This part about scale is still a bit unclear. Our approach do not solve the scaling-up problem.}
\fd{I fully agree with Isabellle.  Adequacy between ecological processes, scales, and meaning for upscaling the information  remain a major challenge.  It would be important here to say how this new approach helps is disentangling the difficulties listed at lines 35-37 in the introduction.}
\mt{Although scaling is always a problem, this doesn't invalidate the statement that our method makes it easier. Think of the first example. This scaling would have been very difficult to do without using Bayesian methods.}
...
\ib{This paragraph is weired. You say that the implementation is very difficult and end the paragraph with decision, practitioners advantage.
I would end the paragraph with positive points about implementation, saying that we still can do a lot with JAGS for instance. And then start a paragraph to briefly talk about challenges in dialogue with practitionners}

The implementation of the model itself can present an obstacle when model complexity increases. In many cases, off-the-shelf software can adequately express the model likelihoods with minimal programming, but more complicated models will require the development of custom programs. Developing such customized code would requires careful model specification, understanding of applied Bayesian methods, and, in some cases, extensive programming. However, the flexibility of our approach, and its transparency with respect to the propagation of uncertainty, will often outweigh the implementation challenges.

Finally, this approach is not just a new methodological tool but is a framework for forecasting species distributions which is fundamentally designed to make the link between modellers and pratitionners, with (1) correctly estimating uncertainties and (2) being updatable to new data or theoretical advances. A scientific approach such as presented here is particularly adapted to make a synthesis of available knowledge and provide robust species distribution forecasts based on information known as being the best of the actual scientific knowledge. It is actually able both to use large databases, valuing the efforts of data collection, and to include models based on the latest theoretical advances, which is essential to decrease the errors due to model specification \citep{Austin2007}. In addition, it would provides practionners and decision-makers with the best possible estimation of uncertainties, with direct applications to risk assessment or to guide the choice where investigating new research and accumulating new data. 
We further argue that the adaptability of our approach is particularly appropriate in our world where collected data and theoretical knowledge is changing as quickly as climate and conservation practices must be adjusted accordingly.


\fd{I am a bit feeling uncomfortable with this section.  Practionners and decision-makers don't have to use (or even fully understand (as they will never get to that level of complexity) what methods underly the predictions) the models.  They use the information known as being the best of the actual scientific knowledge.  As for climate-related variables for CC, the scientific community (as the GIEC for the CC predictions) provide the predictions on SD. The scientific community is responsible for indicating the limitations of the use of the generated information.}


\ib{I would add somewhere that this approach is not just a new methodological tool but is a framework for forecasting species distributions which is fundamentally designed to make the link between modellers and pratitionners, with (1) correctly estimating uncertainties and (2) being adaptable/updatable to new data or theoretical advances.}
